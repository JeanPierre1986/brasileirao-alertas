...
STREAK_MAX_ADJUST = 0.05 # ajuste máximo (+/-5%) por racha actual
STREAK_LOOKBACK = 5      # cuántos partidos recientes se miran para la racha
...
def get_standings():
    """Devuelve {team_name: {position, points}} de la tabla general actual."""
...
def build_team_stats(finished_matches):
    ...
    for m in finished_matches:
        ...
        a_result = "W" if ag > hg else ("D" if hg == ag else "L")

        stats[h]["home"]["gp"] += 1
        stats[h]["home"]["gf"] += hg
        stats[h]["home"]["gc"] += ag
        stats[h]["all_matches"].append({"gf": hg, "gc": ag, "result": h_result})

        stats[a]["away"]["gp"] += 1
        stats[a]["away"]["gf"] += ag
        stats[a]["away"]["gc"] += hg
        stats[a]["all_matches"].append({"gf": ag, "gc": hg, "result": a_result})
...
def current_streak(team_stats):
    """Racha actual: cuenta resultados iguales consecutivos desde el partido
    más reciente. Devuelve (tipo 'W'/'D'/'L', longitud)."""
    recent = team_stats["all_matches"][-STREAK_LOOKBACK:]
    if not recent:
        return None, 0
    ...
    for m in reversed(recent):
        if m["result"] == last_result:
            ...
            break
    return last_result, length


def streak_label(result, length):
    if not result or length == 0:
        return "—"
    letter = {"W": "V", "D": "E", "L": "D"}[result]  # Victoria/Empate/Derrota
    return f"{length}{letter}"


def streak_factor(result, length):
    """Ajuste modesto (+/- hasta STREAK_MAX_ADJUST) sobre el ataque propio del
    equipo, según si viene en racha ganadora o perdedora. Empates no ajustan."""
    if result == "W":
        capped = min(length, STREAK_LOOKBACK)
        return 1 + (capped / STREAK_LOOKBACK) * STREAK_MAX_ADJUST
    if result == "L":
        capped = min(length, STREAK_LOOKBACK)
        return 1 - (capped / STREAK_LOOKBACK) * STREAK_MAX_ADJUST
    return 1.0
...
def predict_match(home, away, team_stats, standings, league_home_avg, league_away_avg, league_generic_avg, h2h_agg):
    ...
    home_streak_result, home_streak_len = current_streak(h) if h else (None, 0)
    away_streak_result, away_streak_len = current_streak(a) if a else (None, 0)
    xg_home *= streak_factor(home_streak_result, home_streak_len)
    xg_away *= streak_factor(away_streak_result, away_streak_len)
    ...
    return {
        ...
        "homePosition": standings.get(home, {}).get("position"),
        "awayPosition": standings.get(away, {}).get("position"),
        "homeStreak": streak_label(home_streak_result, home_streak_len),
        "awayStreak": streak_label(away_streak_result, away_streak_len),
    }
...
def main():
    ...
    for m in upcoming:
        ...
        pred = predict_match(
            m["home"], m["away"], team_stats, standings,
    ...
    output = {
        ...
        "model": "poisson-v3-home_away_splits-recent_form-streak-h2h",
